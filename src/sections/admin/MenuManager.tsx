        ...
        <div className="flex gap-4 w-full lg:w-auto">
          <div className="relative flex-1 lg:w-80">
            ...
            <input 
              ...
              className="w-full pl-11 pr-4 py-3.5 bg-white border border-primary/5 rounded-2xl text-sm font-bold focus:ring-4 focus:ring-primary/10 outline-none shadow-sm transition-all"
            />
          </div>
          <Button 
            onClick={() => {
            ...
            className="py-4 px-6 rounded-2xl font-black gap-2 shadow-xl shadow-primary/10 whitespace-nowrap"
            icon={<FiPlus />}
          >
            TAMBAH
          </Button>
      ...
      {loading ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
          {filteredItems.map((item) => (
            <div
              key={item.id}
              className="bg-white border border-primary/5 p-5 rounded-[2.5rem] shadow-xl shadow-primary/[0.02] hover:shadow-primary/5 transition-all group"
            >
              <div className="flex gap-5">
                ...
                <div className="flex-1 min-w-0 flex flex-col justify-between">
                  <div>
                    ...
                    <h3 className="font-black text-primary truncate group-hover:text-secondary transition-colors text-lg leading-tight">{item.name}</h3>
                    <p className="text-secondary font-bold text-sm mt-1">Rp {item.price.toLocaleString()}</p>
                  </div>
                  <div className="flex gap-2 justify-end mt-4">
                    <button 
                      onClick={() => handleEdit(item)}
                      className="w-10 h-10 rounded-xl bg-primary/5 text-primary/40 flex items-center justify-center hover:bg-primary hover:text-white transition-all shadow-sm"
                    >
                      <FiEdit2 size={16} />
                    </button>
                    <button 
      ...
      <AnimatePresence>
        {isModalOpen && (
          <div className="fixed inset-0 z-[100] flex items-center justify-center p-6 overflow-hidden">
            ...
            <motion.div initial={{ opacity: 0, y: 50, scale: 0.95 }} animate={{ opacity: 1, y: 0, scale: 1 }} exit={{ opacity: 0, y: 50, scale: 0.95 }} className="w-full max-w-xl bg-white p-10 rounded-[3rem] shadow-2xl relative z-10 max-h-[90vh] overflow-y-auto custom-scrollbar">
              <div className="flex justify-between items-center mb-8">
                <h2 className="text-3xl font-black text-primary tracking-tight">{editingItem ? 'Edit Profile' : 'New Creation'}</h2>
